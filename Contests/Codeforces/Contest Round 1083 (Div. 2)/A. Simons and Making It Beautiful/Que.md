...
For a permutation ∗ r of length m, we call an index i (1≤i≤m) ugly if and only if i=max({r1,r2,…,i}). Simons has a permutation p of length n, and he can perform the following operation on p at most once:
...
For permutation [1,2], we can see 1=max({1}) and 2=max({1,2}). So there are two ugly indices.For permutation [2,1], we can see 1≠max({2}) and 2=max({2,1}). So there is only one ugly index.Thus, permutation [2,1] has the minimum count of ugly indices.
